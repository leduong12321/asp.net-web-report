            ...
            string query = @"
                            ...
                            E_TSC1 AS (
                                SELECT A.ID,
                                    ...
                                    H.RENEW_CYCLE_COUNT
                                FROM (SELECT * FROM [HOAPHAT_TSC1_PRD].[dbo].[MDB_EQUIPMENTS] WHERE EQUIPMENT_CFG_ID = @Type AND LAST_MOUNT_DATE >= @FromDay AND LAST_MOUNT_DATE <= @ToDay) A
                                LEFT JOIN (SELECT * FROM [HOAPHAT_TSC1_PRD].[dbo].[REP_EQUIPMENT_MOVEMENTS] WHERE MOVEMENT_DATE >= @FromDay AND MOVEMENT_DATE <= @ToDay) B ON A.NAME = B.EQUIPMENT_NAME
                                LEFT JOIN (SELECT VARIABLE_ID, INTEGER_VALUE, CHAR_VALUE FROM [HOAPHAT_TSC1_PRD].[dbo].[AUX_VALUES] WHERE VARIABLE_ID ='EQP_STATUS_CODES' ) C ON C.INTEGER_VALUE = A.STATUS_CODE 
                                LEFT JOIN (
                                    SELECT * FROM ( SELECT * FROM [HOAPHAT_TSC1_PRD].[dbo].[REP_EQP_MOV_LIFE_ACCOUNTS] WHERE LIFE_ACCOUNT_CODE = 1 ) F
                                    LEFT JOIN (SELECT MOVEMENT_SEQ AS M1, TOTAL_LIFE_FROM_NEW AS TOTAL_WEIGHT_NEW, TOTAL_LIFE_FROM_RENEW AS TOTAL_WEIGHT_RENEW FROM [HOAPHAT_TSC1_PRD].[dbo].[REP_EQP_MOV_LIFE_ACCOUNTS] WHERE LIFE_ACCOUNT_CODE = 5) G ON G.M1 = F.MOVEMENT_SEQ
                                ) E ON E.MOVEMENT_SEQ = B.MOVEMENT_SEQ
                                LEFT JOIN (SELECT EQUIPMENT_ID, CURRENT_LIFE_FROM_NEW, RENEW_CYCLE_COUNT FROM [HOAPHAT_TSC1_PRD].[dbo].[MDB_EQP_LIFE_ACCOUNTS] WHERE LIFE_ACCOUNT_ID = 10 ) H ON H.EQUIPMENT_ID = A.ID 
                            ),
                            E_TSC2 AS (
                                SELECT A.ID,
                                    A.NAME,
                                    ...
                                    E.TOTAL_WEIGHT_RENEW,
                                    H.RENEW_CYCLE_COUNT
                                FROM (SELECT * FROM [HOAPHAT_TSC2_PRD].[dbo].[MDB_EQUIPMENTS] WHERE EQUIPMENT_CFG_ID = @Type AND LAST_MOUNT_DATE >= @FromDay AND LAST_MOUNT_DATE <= @ToDay) A
                                LEFT JOIN (SELECT * FROM [HOAPHAT_TSC2_PRD].[dbo].[REP_EQUIPMENT_MOVEMENTS] WHERE MOVEMENT_DATE >= @FromDay AND MOVEMENT_DATE <= @ToDay) B ON A.NAME = B.EQUIPMENT_NAME
                                LEFT JOIN (SELECT VARIABLE_ID, INTEGER_VALUE, CHAR_VALUE FROM [HOAPHAT_TSC2_PRD].[dbo].[AUX_VALUES] WHERE VARIABLE_ID ='EQP_STATUS_CODES' ) C ON C.INTEGER_VALUE = A.STATUS_CODE 
                                LEFT JOIN (
                                ...
                                LEFT JOIN (SELECT EQUIPMENT_ID, CURRENT_LIFE_FROM_NEW, RENEW_CYCLE_COUNT FROM [HOAPHAT_TSC2_PRD].[dbo].[MDB_EQP_LIFE_ACCOUNTS] WHERE LIFE_ACCOUNT_ID = 10 ) H ON H.EQUIPMENT_ID = A.ID 
                            )
	                        SELECT * FROM E_TSC1 WHERE NAME IS NOT NULL AND MOVEMENT_DATE IS NOT NULL 
                            UNION
